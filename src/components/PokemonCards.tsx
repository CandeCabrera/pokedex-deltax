...
const PokemonCards = ({ url }) => {
  ...
  return (
    <div
      className={`w-[280px] rounded-lg border p-2 cursor-pointer bg-white`}
      onClick={() => navigate(`/pokedex/${pokemonUrl.id}`)}
    >
      <div className="pokeCardTop">
        <h2 style={{fontSize: fontSize}} className="w-full flex justify-center py-2 rounded-lg" style={{background: color}}>{firstLetter(pokemonUrl.name)}</h2>
      ...
      <div className="pokeData">
        ...
        <div className="flex items-center justify-between">
          <span className={`text-[${color}]`}style={{ color: color }}>
            HP:
          </span>
